  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (mode === "create") {
        ...
        // Build edit payload — only send changed fields
      ...
      const data = (await res.json()) as { data?: SystemUser; error?: string };
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                placeholder={mode === "create" ? "Minimum 8 characters" : "Enter to change password"}
...
export default function TeamPage() {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      toast.error("Failed to load team", "Please try refreshing the page.");
    } finally {
      setLoading(false);
    }
  ...
  async function toggleActive(u: SystemUser) {
    try {
      ...
      toast.success(
        ...
        `${u.name} has been ${u.isActive ? "deactivated" : "activated"}.`
      );
    } catch {
  ...
  // ── On create success ─────────────────────────────────────────
  function onCreateSuccess(user: SystemUser) {
    toast.success("User created", `${user.name} has been added to your team.`);
    fetchUsers();
  }

  // ── On edit success ───────────────────────────────────────────
  function onEditSuccess(updated: SystemUser) {
    setUsers((prev) =>
      prev.map((u) => (u.id === updated.id ? updated : u))
    );
    toast.success("User updated", `${updated.name}'s profile has been saved.`);
  }
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-6xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Create, edit, and manage platform user accounts
          </p>
        </div>
        <button
